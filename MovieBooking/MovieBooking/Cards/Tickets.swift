...
//  MovieBooking
//
//  Created by Konstantinos Rizos on 5/11/22.
//
...
struct InfiniteStackView: View {
	...
	var body: some View {
		...
		.zIndex(getIndex() == 0 && offset > 150 ? Double(CGFloat(tickets.count) - getIndex()) - 1 : Double(CGFloat(tickets.count) - getIndex()))
		...
		.gesture(
			DragGesture()
				...
				.onChanged({ value in
					...
					translation = tickets.first?.id == ticket.id
					? translation : 0
